...
-- BEGIN
--     PERFORM pg_notify('log_channel', 'split_and_notify function is being executed.');
--     SELECT json_agg(row_to_json(affected_ids))::jsonb INTO affected_data FROM affected_ids;
    
--     IF affected_data IS NULL THEN
--         PERFORM pg_notify('log_channel', 'No affected data found.');
--         RETURN;
--     END IF;

--     PERFORM pg_notify('log_channel', '1) Length of json array: ' || jsonb_array_length(affected_data));
...
--       --  PERFORM pg_notify('log_channel', '1) Length of data chunk: ' || length(data_chunk)::text);
...
--             PERFORM pg_notify('log_channel', '2) Length of current data chunk: ' || (length(data_chunk) + 2));
--             PERFORM pg_notify('log_channel', '3) Current_Length: ' || (current_length + length(data_chunk) + 2));
--             PERFORM pg_notify('log_channel', '4) jsonb_build_array(json_element): ' || (json_element)::text);
--           --  PERFORM pg_notify('log_channel', 'Sending notification with data chunks: ' || data_chunks::text);
--             -- Uncomment this line to perform the actual notification
...
--         -- PERFORM pg_notify('log_channel', 'Updated data_chunks: ' || data_chunks::text);
--         -- PERFORM pg_notify('log_channel', 'Updated current_length: ' || current_length::text);
--     END LOOP;

--     -- Send any remaining chunks in the array
--     IF jsonb_array_length(data_chunks) > 0 THEN
--         PERFORM pg_notify('log_channel', '5) Sending remaining data chunks. Length is: ' || length(data_chunks::text));
--      --   PERFORM pg_notify('log_channel', '2) Sending remaining data chunks. Length is: ' || jsonb_array_length(data_chunks)::text);
--         PERFORM pg_notify('log_channel', '6) Sending remaining data chunks. Length is: ' || current_length);
--         -- Uncomment this line to perform the actual notification
--         -- PERFORM pg_notify('listing_change', data_chunks::text);
--     END IF;
...
--     PERFORM pg_notify('log_channel', 'split_and_notify function is being executed.');
--     SELECT json_agg(row_to_json(affected_ids))::jsonb INTO affected_data FROM affected_ids;
--     -- PERFORM pg_notify('log_channel', 'Affected data: ' || affected_data::text);
--        PERFORM pg_notify('log_channel', 'Length of json array: ' || (jsonb_array_length(jsonb_array_elements(affected_data))));
--     -- Iterate through each element in the affected_data JSONB array
--     FOR json_element IN SELECT * FROM jsonb_array_elements(affected_data)
--     LOOP
--         data_chunk := json_element::text;
        
--         PERFORM pg_notify('log_channel', '1) Length of data chunk: \n' || length(data_chunk)::text);
...
--     -- Send any remaining chunks in the array
--     IF jsonb_array_length(data_chunks) > 0 THEN
--         PERFORM pg_notify('log_channel', '1) Sending remaining data chunks. Length is: ' || length(data_chunks::text));
--         PERFORM pg_notify('log_channel', '2) Sending remaining data chunks. Length is: ' || jsonb_array_length(data_chunks));
--         PERFORM pg_notify('log_channel', '3) Sending remaining data chunks. Length is: ' || current_length);
--      --   PERFORM pg_notify('listing_change', data_chunks::text);
--     END IF;
