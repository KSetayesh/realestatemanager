...
export class CapRateColumn extends TableColumn {

    private _accessor: PropertyColumnAccessorEnum = PropertyColumnAccessorEnum.CAP_RATE;
    ...
    value(listingWithScenarios: ListingWithScenariosResponseDTO): string | number | boolean {
        return this.getInitialInvestmentDetails(listingWithScenarios).investmentBreakdown.capRate;
    }

    protected _sort(listingWithScenarios: ListingWithScenariosResponseDTO[], sortDirection: SortDirection): ListingWithScenariosResponseDTO[] {
        return listingWithScenarios.sort((a, b) => {
            const aValue = this.getInitialInvestmentDetails(a).investmentBreakdown.capRate;
            const bValue = this.getInitialInvestmentDetails(b).investmentBreakdown.capRate;
